...
const NotificationsPage = () => {
  ...
  useEffect(() => {
    markNotificationsAsSeen();
    // Mark all goal notifications as read in backend
    markAllAsReadMutation();
  }, [markNotificationsAsSeen, markAllAsReadMutation]);

  const handleNotificationClick = (notification) => {
    // Navigate based on notification type
    if (notification.type === "goal_invite") {
      navigate("/learning-goals");
    } else if (notification.learningGoal) {
      navigate(`/learning-goals/${notification.learningGoal}`);
    }
  };
  ...
  const goalInvites = notifications.filter((n) => n.type === "goal_invite");
  const quizNotifications = notifications.filter(
    (n) => n.type === "quiz_completed"
  );
  const otherNotifications = notifications.filter(
    (n) => n.type !== "goal_invite" && n.type !== "quiz_completed"
  );

  const isLoading = friendRequestsLoading || notificationsLoading;
      ...
      <div className="container mx-auto max-w-4xl space-y-6 sm:space-y-8">
        ...
        {isLoading ? (
          ...
          <>
            ...
            {goalInvites.length > 0 && (
              <section className="space-y-3 sm:space-y-4 animate-fade-in">
                <h2 className="text-lg sm:text-xl font-semibold flex items-center gap-2 flex-wrap animate-slide-in-left">
                  ...
                  <span>Learning Goal Invitations</span>
                  <span className="badge badge-primary text-xs sm:text-sm">
                    {goalInvites.length}
                  </span>
                </h2>
                ...
                <div className="space-y-2 sm:space-y-3">
                  {goalInvites.map((notification, index) => (
                    <div
                      key={notification._id}
                      className={`card bg-base-200 hover-lift hover-glow border ${
                        notification.read
                          ? "border-base-300/20"
                          : "border-blue-500/50"
                      } card-animate cursor-pointer`}
                      style={{ animationDelay: `${index * 0.1}s` }}
            ...
            {/* QUIZ COMPLETED NOTIFICATIONS */}
            {quizNotifications.length > 0 && (
              <section className="space-y-3 sm:space-y-4 animate-fade-in">
                ...
                <div className="space-y-2 sm:space-y-3">
                  {quizNotifications.map((notification, index) => (
                    <div
                      key={notification._id}
                      className={`card bg-base-200 hover-lift border ${
                        notification.read
                          ? "border-base-300/20"
                          : "border-green-500/50"
                      } card-animate cursor-pointer`}
            ...
            {/* OTHER NOTIFICATIONS (goal accepted, completed) */}
            {otherNotifications.length > 0 && (
              <section className="space-y-3 sm:space-y-4 animate-fade-in">
                <h2 className="text-lg sm:text-xl font-semibold flex items-center gap-2 animate-slide-in-left">
                  <BellIcon className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />
                  Updates
                </h2>
                          ...
                          <div className="avatar flex-shrink-0 mt-1">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full">
                              <img
                                ...
                                alt={notification.sender?.fullName || "System"}
                                className="w-full h-full object-cover rounded-full"
                              />
                            </div>
                          </div>
