...
export async function searchUsers(req, res) {
  try {
    ...
    const users = await User.find({
      ...
      .select(
        "fullName username profilePic bio nativeLanguage learningLanguage location friends"
      )
...
/**
 * Generate random avatars for all users without profile pictures
 * POST /api/users/generate-avatars
 */
export async function generateAvatarsForExistingUsers(req, res) {
  try {
    ...
    for (const user of usersWithoutAvatar) {
      ...
      const randomSeed =
        user._id.toString() + Math.random().toString(36).substring(2, 15);
    ...
    console.log(`✅ Generated avatars for ${updatedUsers.length} users`);

    res.status(200).json({
      message: `Successfully generated avatars for ${updatedUsers.length} users`,
      updated: updatedUsers.length,
      users: updatedUsers,
    });
  } catch (error) {
    console.error("Error in generateAvatarsForExistingUsers:", error.message);
    res.status(500).json({ message: "Internal server error" });
  }
}
